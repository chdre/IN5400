...
#######################################################################################################################
def produce_cnn_features(myDataLoader, device):

    # set path to pre trained models weights
    os.environ['TORCH_HOME'] = myDataLoader.data_dir + '\\models\\CNN_model'

    #restore pretrained CNN model
    ...
    saveDirTrain = myDataLoader.data_dir + '/Train2017_cnn_features/'
    saveDirVal   = myDataLoader.data_dir + '/Val2017_cnn_features/'
    ...
    if (os.listdir(saveDirTrain) == []) or (os.listdir(saveDirVal) == []):
        # Store all fc layers as pickel objects.
        for dataDict in tqdm(myDataLoader.myDataLoaderVal, desc='', leave=True, mininterval=0.01):
            ...
            with torch.no_grad():
                cnn_features = model(images)
            saveFcAsPickle(cnn_features.detach().cpu().numpy(), img_paths, fileName, captions, vocabularyDict, saveDirVal)
        ...
        for dataDict in tqdm(myDataLoader.myDataLoaderTrain, desc='', leave=True, mininterval=0.01):
            ...
            with torch.no_grad():
                cnn_features = model(images)
            saveFcAsPickle(cnn_features.detach().cpu().numpy(), img_paths, fileName, captions, vocabularyDict, saveDirTrain)
...
###########################################################################################################
def saveFcAsPickle(cnn_features, path_list, fileName, original_captions_list, vocabularyDict, saveDir):
    ...
    for ii in range(len(path_list)):
        ...
        dataDict = {}
        dataDict['cnn_features']      = cnn_features[ii, :]
